...
function ivEncryptionResponse(ti, cmdCtr, sesAuthEncKey) {
  const ivData = AES.encrypt(CryptoJS.enc.Hex.parse('5AA5' + ti + cmdCtr + '0000000000000000'), CryptoJS.enc.Hex.parse(sesAuthEncKey), {
    mode: CryptoJS.mode.ECB,
    // iv: CryptoJS.enc.Hex.parse("00000000000000000000000000000000"),
    keySize: 128 / 8,
    padding: CryptoJS.pad.NoPadding,
...
/**
 * Set the ADPU command function to communicate with the NFC antenna on the device
 *
 * @param {function} APDUCommandHandler
 * @returns void
 */
Ntag424.setSendAPDUCommand = APDUCommandHandler => {
  Ntag424.sendAPDUCommand = APDUCommandHandler;
};
...
Ntag424.calcMac = function (commandData) {
  const commandMac = CryptoJS.CMAC(CryptoJS.enc.Hex.parse(Ntag424.sesAuthMacKey), CryptoJS.enc.Hex.parse(commandData));
...
Ntag424.getCardUid = async () => {
  ...
  const resDataDec = AES.decrypt({ ciphertext: CryptoJS.enc.Hex.parse(resDataEnc) }, CryptoJS.enc.Hex.parse(Ntag424.sesAuthEncKey), {
    padding: CryptoJS.pad.NoPadding,
    mode: CryptoJS.mode.CBC,
    iv: CryptoJS.enc.Hex.parse(iv),
    keySize: 128 / 8,
